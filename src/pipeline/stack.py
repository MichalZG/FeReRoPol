...
class Stack(PipelineBase):
	...
	def _calculate_mid_time(self, start_im, end_im):
		...
		start_datetime = Time("1988-10-26", format='isot')
		end_datetime = Time("1988-10-26", format='isot')
	...
	def _sort_stack_list(self, stack_lists):
		for savart_name, savart_list in stack_lists.items():
			stack_lists[savart_name] = sorted(savart_list, key=lambda x: x.jd)
	...
	def _create_stack_lists(self, names_of_savarts):
		...
		for image in self.images_list:
			if str(image.savart) in stack_lists:
				stack_lists[str(image.savart)].append(image)

		stack_lists = self._sort_stack_list(stack_lists)
	...
	def process(self):
		...
		for savart_name, stack_list in stack_lists.items():
			for i, chunk in enumerate(stack_list):
				stack_name = '{}_{:03d}{}'.format(savart_name, i+1,
					self.config_section.get('pattern').replace('*', ''))
