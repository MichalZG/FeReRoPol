...
class Savartphot(PipelineBase):
    ...
    def _create_images_list(self, work_dir):
        ...
        images_list = sorted(images_list, key = lambda x: x.jd)
    ...
    def process(self):
        ...
        for image in self.images_list:
            ...
            apertures, sigma_values_table = self._create_apertures(
                image, self.stars_coordinates[str(image.savart)],
                image.shape)
            ...
            if image.savart not in self.measurements:
                self.measurements[image.savart] = [SavartCounts(image.savart, image.jd,
                        counts_tab, counts_error_tab)]
            else:
                self.measurements[image.savart].append(SavartCounts(image.savart, image.jd,
                        counts_tab, counts_error_tab))
    ...
    def _check_if_eq_len(self):
        self.info('Equality of measurments checking have been started')
        measurements_counts = [len(m) for m in self.measurements.values()]
        min_val = min(measurements_counts)
        if not measurements_counts.count(measurements_counts[0]) == measurements_counts:
            self.warning('Measurments are not equal, cut all to: {}'.format(min_val))
            for k, v in self.measurements.items():
                self.measurements[k] = v[:min_val]
        else:
            self.info('Measurments are equal')
    ...
    def _save_polarizaton_results(self):
        ...
        min_len = self._check_if_eq_len()
        # phi_tab = [[float(x), float(x)+90.] for x in self.measurements.keys()]
        # phi_tab = [i for l in phi_tab for i in l]
        ...
        for idx in range(min_len):
            ...
            for k, v in self.measurements.items():
                ...
                phi_tab.append(float(k))
                phi_tab.append(float(k)+90)
                i_tab.append(v[idx].counts1)
